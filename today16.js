...
async function fetchData() {
    try {
        ...
        yesterday.setDate(yesterday.getDate() - 2); // Move to yesterday
        ...
        // Ensure the data is present and is in correct format before proceeding
        ...
        // Filter for news articles from yesterday
        const yesterdayNews = newsData.articles.filter(article => {
            const articleDate = new Date(article.publishedAt);
            return articleDate >= startOfYesterday && articleDate <= endOfYesterday;
        });

        // Filter for curator posts from yesterday
        const yesterdayPosts = postsData.posts.filter(post => {
            const postDate = new Date(post.source_created_at);
            return postDate >= startOfYesterday && postDate <= endOfYesterday;
        });


        // Combine filtered news and posts into a single timeline
        const combinedData = [...yesterdayNews, ...yesterdayPosts].sort((a, b) => {
            // Assuming both have either `publishedAt` or `source_created_at` as fields for date comparison
            const dateA = new Date(a.publishedAt || a.created_at || a.source_created_at);
            const dateB = new Date(b.publishedAt || b.created_at || b.source_created_at);
            return dateB - dateA;
        });
