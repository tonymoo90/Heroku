...
async function fetchData() {
    try {
        ...
        // Filter for news articles from two days ago
        const twoDaysAgoNews = newsData.articles.filter(article => {
            const articleDate = new Date(article.publishedAt);
            return articleDate >= startOfTwoDaysAgo && articleDate <= endOfTwoDaysAgo;
        });

        // Filter for curator posts from two days ago
        const twoDaysAgoPosts = postsData.posts.filter(post => {
            const postDate = new Date(post.source_created_at);
            return postDate >= startOfTwoDaysAgo && postDate <= endOfTwoDaysAgo;
        });


        // Combine news and posts into a single timeline
        const combinedData = [...newsData.articles, ...postsData.posts].sort(
